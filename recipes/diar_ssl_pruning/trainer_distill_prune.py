...
class Trainer(BaseTrainer):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_reg and self.state.epochs_trained >= self.pre_train_epochs:
            ...
            cur_expected_sparsity = 1.0 - self.unwrap_model.student_model.get_num_params() / self.original_num_params
            loss_reg = (
    ...
    def validation_epoch_end(self, validation_epoch_output):
        ...
        logger.info(
            f"Validation Loss | Loss_distill | Loss_reg on epoch {self.state.epochs_trained}: {round(Loss_val, 3)} | {round(Loss_distill_val, 3)} | {round(Loss_reg_val, 3)}"
        )
