...
class Trainer:
    ...
    def _initialize_exp_dirs_and_paths(self, config):
        ...
        self.ranked_checkpoints_dir = self.exp_dir / "ranked_checkpoints"
    ...
    def _save_ranked_model_checkpoint(self, destination):
        temporary = destination.with_name(f".{destination.name}.partial")
        if temporary.exists():
            self._remove_path(temporary)
        temporary.mkdir(parents=True, exist_ok=True)

        model_path = temporary / "pytorch_model.bin"
        partial_model_path = temporary / "pytorch_model.partial.bin"
        state_dict = self.accelerator.get_state_dict(self.model)
        self.accelerator.save(state_dict, partial_model_path)
        partial_model_path.replace(model_path)
        self._write_checkpoint_completion_marker(temporary)
        self._publish_checkpoint_directory(temporary, destination, self._is_ranked_checkpoint_complete)

    def _ranked_checkpoint_paths(self):
        paths = sorted(self.ranked_checkpoints_dir.glob("epoch_" + ("[0-9]" * 4)))
        return [path for path in paths if self._is_ranked_checkpoint_complete(path)]
    ...
    def _save_ranked_checkpoint(self, epoch: int, score: float) -> None:
        ...
        if self.ranked_checkpoint_count <= 0:
            return

        self.ranked_checkpoints_dir.mkdir(parents=True, exist_ok=True)
        index_path = self.ranked_checkpoints_dir / "index.json"
        ...
        for record in records:
            try:
                ...
                continue
            checkpoint_dir = self.ranked_checkpoints_dir / f"epoch_{record_epoch:04d}"
            if self._is_ranked_checkpoint_complete(checkpoint_dir):
                valid_records.append({"epoch": record_epoch, "score": record_score})

        records = [record for record in valid_records if record["epoch"] != epoch]
        records.append({"epoch": epoch, "score": float(score)})
        records.sort(key=lambda record: record["score"], reverse=self.save_max_score)

        retained = records[: self.ranked_checkpoint_count]
        retained_epochs = {record["epoch"] for record in retained}
        if epoch in retained_epochs:
            self._save_ranked_model_checkpoint(self.ranked_checkpoints_dir / f"epoch_{epoch:04d}")

        for record in retained:
            checkpoint_dir = self.ranked_checkpoints_dir / f"epoch_{record['epoch']:04d}"
            if not self._is_ranked_checkpoint_complete(checkpoint_dir):
                raise RuntimeError(f"Ranked checkpoint {checkpoint_dir.as_posix()} is incomplete.")

        temporary_index = index_path.with_suffix(".partial.json")
        with temporary_index.open("w", encoding="utf-8") as handle:
            handle.write(json.dumps(retained, indent=2) + "\n")
            handle.flush()
            os.fsync(handle.fileno())
        temporary_index.replace(index_path)
        self._fsync_directory(self.ranked_checkpoints_dir)

        retained_paths = {self.ranked_checkpoints_dir / f"epoch_{record['epoch']:04d}" for record in retained}
        for checkpoint_dir in self._ranked_checkpoint_paths():
            if checkpoint_dir not in retained_paths:
                self._remove_path(checkpoint_dir)
    ...
    def create_lr_decay_scheduler(self):
        self.lr_decay_scheduler_small = ReduceLROnPlateau(
            optimizer=self.optimizer_small, mode="min", factor=0.95, patience=self.lr_decay_patience, min_lr=1e-8
        )
        self.lr_decay_scheduler_big = ReduceLROnPlateau(
            optimizer=self.optimizer_big, mode="min", factor=0.95, patience=self.lr_decay_patience, min_lr=1e-8
        )
    ...
    def create_lr_one_cycle_scheduler(self, max_steps):
        self.lr_one_cycle_scheduler_small = OneCycleLR(
            optimizer=self.optimizer_small, max_lr=self.get_optimizer_lr(self.optimizer_small), total_steps=max_steps
        )
        self.lr_one_cycle_scheduler_big = OneCycleLR(
            optimizer=self.optimizer_big, max_lr=self.get_optimizer_lr(self.optimizer_big), total_steps=max_steps
        )
        ...
        for key in loss_keys:
            ...
            if self.accelerator.is_local_main_process:
                ...
                self.writer.add_scalar(
                    "Train_Epoch/lr_small", get_rate(self.optimizer_small), self.state.epochs_trained
                )
                self.writer.add_scalar("Train_Epoch/lr_big", get_rate(self.optimizer_big), self.state.epochs_trained)
